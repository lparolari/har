...
As reported in \cite{stisen2015smart}, three major types of heterogeneties yield impairments in HAR:
\begin{itemize}
...
For our application we decided to use the \textit{Heterogenity Dataset (HD)} \cite{HETEROGENEITY} described in
...
\begin{align}
  ...
  f'_{\theta}(\x) &= \text{ReLU}(\boldsymbol{W_{1}}\x + \boldsymbol{b_{1}})
\end{align}
...
\begin{align}
  ...
  g'_{\theta}(\y) &= \text{ReLU}(\boldsymbol{W_{3}}\y + \boldsymbol{b_{3}})
\end{align}
